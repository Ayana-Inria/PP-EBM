...
from modules.unet.unet_parts import DoubleConv, Down, Up
...
class Unet(nn.Module):
    ...
    def __init__(self, hidden_dims: List[int], in_channels: int, device, last_channels: int = None):
        ...
        for i, out_channels in enumerate(hidden_dims[::-1][1:]):
            if last_channels is not None and i == len(hidden_dims) - 2:
                self.ascending_path.append(
                    Up(in_channels, last_channels, in_channels_skip=out_channels))
                in_channels = last_channels
            else:
                self.ascending_path.append(
                    Up(in_channels, out_channels, in_channels_skip=out_channels))
                in_channels = out_channels
