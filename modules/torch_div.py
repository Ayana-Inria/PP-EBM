...
def torch_divergence(f: Tensor, indexing='xy'):
    ...
    spacing = 1.0
    if indexing == "xy":
        stack = [torch.gradient(f[:, num_dims - i - 1], dim=i + 1)[0]
                 for i in range(num_dims)]
    elif indexing == "ij":
        stack = [torch.gradient(f[:, i], dim=i + 1)[0]
                 for i in range(num_dims)]
    else:
        raise ValueError
...
class Divergence(Module):
    ...
    def __init__(self, div_channels: List[int], mask_channel: int, sigmoid_on_mask: bool):
        super(Divergence, self).__init__()
        self.div_channels = div_channels
        self.mask_channel = mask_channel
        self.sigmoid_on_mask = sigmoid_on_mask

    def forward(self, x: Tensor):
        div_c = torch_divergence(x[:, self.div_channels], indexing='ij')

        mask = x[:, [self.mask_channel]]
        if self.sigmoid_on_mask:
            mask = torch.sigmoid(mask)

        return div_c * mask


class NegativeModule(Module):
    def forward(self, x: Tensor):
        return -x
