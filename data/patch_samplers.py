...
class UniformSampler(PatchSampler):
    ...
    def initialise(self, patch_files, label_files, meta_files):
        ...
        samples_per_image = (
            pixel_count / np.sum(pixel_count)) * self.n_patches
        self.sample_density_per_image = samples_per_image / \
            np.sum(samples_per_image)
        self.sample_density_per_image[np.isnan(
            self.sample_density_per_image)] = 0.0
...
class ObjectSampler(PatchSampler):
    ...
    def initialise(self, patch_files, label_files, meta_files):
        ...
        samples_per_image = (
            vehicles_count / np.sum(vehicles_count)) * self.n_patches
        self.sample_density_per_image = samples_per_image / \
            np.sum(samples_per_image)
...
class DensitySampler(PatchSampler):
    ...
    def initialise(self, patch_files, label_files, meta_files):
        ...
        self.sample_density_per_image = summed_density_per_image / \
            np.sum(summed_density_per_image)
...
class MixedSampler(PatchSampler):
    ...
    def initialise(self, patch_files, label_files, meta_files):
        ...
        weighted_samplers_densities = [
            w * s.sample_density_per_image for s, w in zip(self.samplers, self.weights)]
        self.sample_density_per_image = np.sum(
            weighted_samplers_densities, axis=0)
        assert len(self.sample_density_per_image) > 0
        # just in case
        self.sample_density_per_image = self.sample_density_per_image / \
            np.sum(self.sample_density_per_image)
