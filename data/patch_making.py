...
from base.data import fetch_data_paths, check_data_match, get_dataset_base_path
...
def make_patch_dataset(new_dataset: str, source_dataset: str, config: Dict, rng: np.random.Generator,
    ...
    n_patches_val = config["data_loader"]['patch_maker_params'].get(
        'n_patches_val', n_patches // 2)
    ...
    for subset in (['train', 'val'] if make_val else ['train']):
        logging.info(f"(re)generating {subset} patches data")

        sampler = MixedSampler(
            n_patches=n_patches,
            samplers=[
                UniformSampler(n_patches=n_patches,
                               patch_size=patch_size, rng=rng),
                ObjectSampler(n_patches=n_patches,
                              patch_size=patch_size, rng=rng, sigma=sigma)
            ],
            weights=[
        ...
        _make_patches(
            ...
            sampler=sampler,
            n_patches=n_patches if subset == 'train' else n_patches_val,
            patch_size=patch_size,
            rng=rng,
            multiprocess=True,
            clear=True,
            verbose=0,
            extract_patch_args=config["data_loader"].get(
                'extract_patch_args', {})
        )
...
def _make_patches(source_dataset: str, subset: str, new_dataset: str, sampler: PatchSampler, n_patches: int,
                  patch_size: int,
                  rng: np.random.Generator, multiprocess=True, clear=False, verbose=1, extract_patch_args: Dict = {}):
    paths = fetch_data_paths(source_dataset, subset)
    sampler.initialise(
        paths['images'], paths['annotations'], paths['metadata'])

    try:
        samples_per_image = rng.multinomial(
            n=n_patches, pvals=sampler.sample_density_per_image)
    except Exception as e:
        logging.error(
            f"sampling failed with densities {sampler.sample_density_per_image} and {n_patches=}")
        raise e

    new_dataset_path = os.path.join(
        get_dataset_base_path(), new_dataset, subset)
    make_if_not_exist(new_dataset_path, recursive=True)
    make_if_not_exist([os.path.join(new_dataset_path, d)
                      for d in ['images', 'annotations', 'metadata']])
    ...
    fun = partial(_make_one_patch, sampler=sampler, patch_size=patch_size, data_dest=new_dataset_path,
                  **extract_patch_args)
    it = zip(range(len(paths['images'])), samples_per_image,
             paths['images'], paths['annotations'], paths['metadata'])
    if multiprocess:
        with Pool() as p:
            p.starmap(fun, it)
...
def _make_one_patch(i, n_local_patches, patch_path, label_path, meta_path, sampler, patch_size, data_dest,
    ...
    for k in range(n_local_patches):
        ...
        anchor = sampler.sample_patch_center(
            image_id=i, shape=shape, centers=centers)
        patch, tl_anchor, centers_offset = extract_patch(
            image=image, center_anchor=anchor, patch_size=patch_size, **extract_patch_args
        )
        patch_centers = []
        patch_parameters = []
        patch_categories = []
        ...
        plt.imsave(os.path.join(data_dest, 'images',
                   f"{patch_name}.png"), patch)
