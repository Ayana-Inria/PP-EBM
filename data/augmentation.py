...
def medium_aug(hist_match_images_paths=None):
    return [
        ...
        alb.RandomRotate90(),
        # alb.Rotate(limit=180, border_mode=cv2.BORDER_CONSTANT, value=0),
        alb.Flip(),
        (
            alb.HistogramMatching(
                reference_images=hist_match_images_paths, blend_ratio=(0.1, 0.75))
...
def medium_aug_static(hist_match_images_paths=None):
    return [
        alb.FromFloat(dtype='uint8', always_apply=True),
        # alb.RandomRotate90(),
        # alb.Rotate(limit=180, border_mode=cv2.BORDER_CONSTANT, value=0),
...
def strong_aug_static(hist_match_images_paths=None):
    return [
        alb.FromFloat(dtype='uint8', always_apply=True),
        # alb.RandomRotate90(),
        # alb.Rotate(limit=180, border_mode=cv2.BORDER_CONSTANT, value=0),
...
def aggressive_aug_static(hist_match_images_paths=None):
    return [
        alb.FromFloat(dtype='uint8', always_apply=True),
        # alb.RandomRotate90(),
        # alb.Rotate(limit=180, border_mode=cv2.BORDER_CONSTANT, value=0),
        ...
        (
            alb.HistogramMatching(
                reference_images=hist_match_images_paths, blend_ratio=(0.4, 0.75))
            if hist_match_images_paths is not None else alb.NoOp()
        ),
        alb.RandomShadow(),
        alb.RandomFog(),
        ...
        alb.RandomBrightnessContrast(brightness_limit=0.5, contrast_limit=0.5),
        alb.OneOf([
            alb.CLAHE(),
            alb.RGBShift(),
            alb.ToGray()
        ...
        # alb.Downscale(scale_min=0.9, scale_max=0.9),
        alb.OneOf([
            alb.MedianBlur(blur_limit=7, p=0.2),
            alb.Blur(blur_limit=7, p=0.2),
        ], p=0.4),
...
class DataAugment:
    ...
    def __post_init__(self):
        ...
        elif self.aug_level == 'medium_static':
            augs = medium_aug_static(self.hist_match_images_paths)
        elif self.aug_level == 'strong_static':
            augs = strong_aug_static(self.hist_match_images_paths)
        elif self.aug_level == 'aggressive_static':
            augs = aggressive_aug_static(self.hist_match_images_paths)
        else:
            raise ValueError
        ...
        self.transformer = alb.Compose(
            ...
            keypoint_params=alb.KeypointParams(
                format='xya', remove_invisible=False, angle_in_degrees=False)
        )
    ...
    def transform(self, patch, centers, params):
        ...
        if n_points == 0:
            ...
            angles = params[:, 2]
            keypoints = np.stack(
                [centers[:, 1], centers[:, 0], angles], axis=-1)
        ...
        if n_points == 0:
            ...
            new_centers = np.stack(
                [transformed_keypoints[:, 1], transformed_keypoints[:, 0]], axis=-1).astype(int)
            new_params = np.stack(
                [params[:, 0], params[:, 1], new_angles], axis=-1)
