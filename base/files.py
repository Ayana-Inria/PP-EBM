...
def make_if_not_exist(path: Union[str, List[str]], recursive=False, recursive_limit: int = 4):
        ...
        if not os.path.exists(p):
            if recursive:
                if recursive_limit == 0:
                    raise RuntimeError(
                        f"Ooops, looks like you are creating quite a bunch of directories, is the path correct ? {p}")
