...
class Logger:
    ...
    def update(self, epoch: int, metrics: Dict[str, float], prefix=''):
        ...
        append_lists_in_dict(
            self.log, {prefix + k: v for k, v in metrics.items()})

        if self.checkpoint_interval is not None and epoch % self.checkpoint_interval == 0:
            last_checkpoint_path = glob.glob(
                os.path.join(self.save_dir, 'checkpoint_*.pt'))
            if len(last_checkpoint_path) > 0:
                for p in last_checkpoint_path:
                    os.remove(p)
            torch.save(self.model.state_dict(), os.path.join(
                self.save_dir, f'checkpoint_{epoch:04}.pt'))
